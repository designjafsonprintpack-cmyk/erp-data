...
import { workflowStageSchema, workflowStageUpdateSchema } from '@/lib/schemas/settingsConfig'
...
export const POST = withErrorHandling(async function POST(req: NextRequest) {
  ...
  const parsed = await parseBody(req, workflowStageSchema)
  if ('error' in parsed) return parsed.error
  const body = parsed.data
  const { data, error } = await supabase.from('workflow_stages' as any)
...
export const PATCH = withErrorHandling(async function PATCH(req: NextRequest) {
  ...
  if (denied) return denied
  const parsed = await parseBody(req, workflowStageUpdateSchema)
  if ('error' in parsed) return parsed.error
  const { id, ...fields } = parsed.data
  const { data, error } = await supabase.from('workflow_stages' as any).update(fields).eq('id', id).eq('company_id', companyId).select().single()
  if (error) return NextResponse.json({ error: error.message }, { status: 500 })
  return NextResponse.json({ data })
})
...
export const DELETE = withErrorHandling(async function DELETE(req: NextRequest) {
  ...
  const { error } = await supabase.from('workflow_stages' as any)
