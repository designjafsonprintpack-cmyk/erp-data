...
export const GET = withErrorHandling(async function GET(_req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const { data, error } = await supabase.from('automation_rule_runs' as any)
    .select('id, triggered_for, action_taken, created_at')
    .eq('rule_id', params.id).eq('company_id', companyId)
