...
export async function GET() {
  ...
  const { data, error } = await supabase.from('machines' as any).select('*').is('deleted_at', null).order('machine_type').order('name')
...
export async function PATCH(req: NextRequest) {
  ...
  const { data, error } = await supabase.from('machines' as any).update(fields).eq('id', id).select().single()
  if (error) return NextResponse.json({ error: error.message }, { status: 500 })
  return NextResponse.json({ data })
}
