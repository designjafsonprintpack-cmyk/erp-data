...
export const PATCH = withErrorHandling(async function PATCH(req: NextRequest, { params }: { params: { resource: string } }) {
  const table = VALID[params.resource]
  if (!table) return NextResponse.json({ error: 'Invalid resource' }, { status: 400 })
  const supabase = createSupabaseServerClient()
  const { data: { user } } = await supabase.auth.getUser()
...
export const DELETE = withErrorHandling(async function DELETE(req: NextRequest, { params }: { params: { resource: string } }) {
  const table = VALID[params.resource]
  if (!table) return NextResponse.json({ error: 'Invalid resource' }, { status: 400 })
  const supabase = createSupabaseServerClient()
  const { data: { user } } = await supabase.auth.getUser()
