...
export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const { data, error } = await supabase.from('sales_orders' as any).update(body).eq('id', params.id).select().single()
...
export async function DELETE(_: NextRequest, { params }: { params: { id: string } }) {
  ...
  await supabase.from('sales_orders' as any)
    .update({ deleted_at: new Date().toISOString(), status: 'cancelled' }).eq('id', params.id)
